<template>
  <div id="app">
    <router-view
      ...
      :countriesCorrect="countriesCorrect"
...
export default {
  ...
  mounted() {
    fetch("https://restcountries.com/v2/all") //API
      .then((res) => res.json())
    ...
    eventBus.$on("map-country-selected", (alpha2Code) => {
      const selectedCountry = this.countries.find(
        (country) => country.alpha2Code.toLowerCase() === alpha2Code
      );
  ...
  methods: {
    ...
    getRandomCountry(countriesRemaining) {
      this.randomCountry =
        countriesRemaining[
          Math.floor(Math.random() * countriesRemaining.length)
        ];
      this.result = null;
    },
    ...
    checkAnswer() {
      if (
        this.randomCountry.alpha3Code === this.countryListSelected.alpha3Code
      ) {
        this.countriesCorrect = [
          ...this.countriesCorrect,
          this.countryListSelected,
        ];
        const index = this.countriesRemaining.indexOf(this.countryListSelected);
        this.countriesRemaining.splice(index, 1);
        this.result = "correct";
        this.currentUser.results[this.randomCountry.alpha3Code]["flagGame"] =
          "true";
        if (this.loggedIn && localStorage.getItem("jwt")) {
          // this.checkToken();
          UserService.updateUserResults(
            this.currentUser,
            localStorage.getItem("jwt")
          )
            .then((updatedUser) =>
              eventBus.$emit("country-correct", updatedUser)
            )
            .catch((response) => {
    ...
    setCorrectCountries() {
      for (const country of this.countries) {
        if (
          this.currentUser.results[country.alpha3Code]["flagGame"] === "true"
        ) {
          const index = this.countriesRemaining.indexOf(country);
          if (index > -1) {
            this.countriesCorrect.push(
              this.countriesRemaining.splice(index, 1)[0]
            );
          }
        }
      }
